...
public:
   ...
   /* //asigna la funcion que va ser llamada cada frame
    ...
    /*void setFrameProcessor(FeatureTracker* frameProcessorptr){
        process=0;
        frameProcessor=frameProcessorptr;
        callProcess();
    ...
    void setFrameProcessor(void (*frameProcessingCallback)(cv::Mat&, cv::Mat&)){
        ...
        frameProcessor=0;//invalida la funcion
        process= frameProcessingCallback;//esta es la funcion que sera llamada
        callProcess();
    ...
    void run(){
        ...
        while(!isStopped()){
            ...
            if(callIt){
                ...
                if(process) process(frame, output);
                else if(frameProcessor)frameProcessor->process(frame,output);//si estoy usando la clase frame
                fnumber++;//incrementa el numero de frame
